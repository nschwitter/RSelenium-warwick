...
How can we find the tags and attributes of the information we are interested in? Through our browser, we can look at the HTML source code of any website and also inspect specific elements. There are also tools to make our life easier.  ***SelectorGadget*** is a useful, open source JavaScript bookmarklet that allows you to interactively figure out what CSS selector you need to extract part of the page.

Check out its [webpage](https://selectorgadget.com/) to get it. To use SelectorGadget, you click on the element that you want to select and iteratively select and de-select elements. SelectorGadget will make a guess at what CSS selector you might want.
...
```{r}
library(RSelenium)
driver <- rsDriver(browser = "chrome",chromever = "108.0.5359.71",port = 4448L)
